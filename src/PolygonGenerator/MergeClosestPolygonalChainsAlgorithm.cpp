...
typedef CgalTypes::Polygon PolygonalChain;

CgalTypes::Number minimum_squared_distance(const PolygonalChain &a, const PolygonalChain &b) {
    CgalTypes::Number minimum_squared_distance = std::numeric_limits<double>::max();
    for (const CgalTypes::Segment edge_a: a.edges()) {
        for (const CgalTypes::Segment edge_b: b.edges()) {
            if (const CgalTypes::Number squared_distance = CGAL::squared_distance(edge_a, edge_b);
...
CgalTypes::Number minimum_squared_distance(const CgalTypes::Point &a, const PolygonalChain &b) {
    CgalTypes::Number minimum_squared_distance = std::numeric_limits<double>::max();
    for (const CgalTypes::Segment edge: b.edges()) {
        if (const CgalTypes::Number squared_distance = CGAL::squared_distance(a, edge);
            squared_distance < minimum_squared_distance) {
            minimum_squared_distance = squared_distance;
        }
    }
...
template<typename Container>
CgalTypes::Polygon mergeClosestChainsAlgorithm(const Container &points) {
